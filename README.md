...
`id_arrived` - передаваемая переменная при запуске код

### Как же их получить?

`id_arrived` передаётся в запуске. Посмотреть как передовать показано в следущем разделе.
...
`Если id_arrived не передаётся, то будут сохранятся фотографии с последнего запуска`
...
Пример: API_KEY. Чтобы получить АPI ключь. Зарегистрируйтесь на [этом сайте](https://api.nasa.gov/#apod). И получите API ключь.
